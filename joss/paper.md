...
## Software Design
The GEFF specification emphasizes simplicity over optimization. It would be infeasible to make a single graph format, or even tracking format, that was optimized for all stages of the tracking pipeline from algorithm development to visualization. Implementing a simple exchange format allows a variety of tools with different use cases to all exchange information, while retaining their optimized internal formats. Additionally, focusing on an exchange format allows cross-language compatibility. In addition to a hand-written specification, the GEFF specification package provides Pydantic classes and a JSON schema for creating and validating the metadata properties.
...
The `_graph_libs` currently implements three graph backends. `networkx` and `rustworkx` are two of the most common Python graph libraries, making GEFF adoption simple for most Python programmers. `spatial_graph` is a newer graph library with improved efficiency when searching for all graph elements in a spatial region, which is extremely useful for visualization applications that want to leverage lazy loading of chunked data for graph as well as image data. While the Python implementation contains significant internal typing and abstraction logic that reduces code duplication and enhances maintainability, the external API is quite simple; most users will only need to use the public `read` and `write` functions.
