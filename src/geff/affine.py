...
class Affine(BaseModel):
    ...
    Attributes:
        matrix: Homogeneous transformation matrix of shape (ndim+1, ndim+1)
    """
    model_config = ConfigDict(arbitrary_types_allowed=True)

    matrix: NDArray[np.floating] = Field(..., description="Homogeneous transformation matrix (ndim+1, ndim+1)")
    ...
    def validate_matrix(self) -> 'Affine':
        ...
        if matrix.shape[0] < 2:
            raise ValueError(f"Matrix must be at least 2x2 for homogeneous coordinates, got {matrix.shape}")
        
        # Check that bottom row is [0, 0, ..., 1]
        expected_bottom_row = np.zeros(matrix.shape[1])
        expected_bottom_row[-1] = 1.0
        
        if not np.allclose(matrix[-1, :], expected_bottom_row):
            raise ValueError(
