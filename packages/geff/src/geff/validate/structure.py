...
def validate_structure(input: StoreLike | InMemoryGeff) -> None:
    """Ensure that the structure of the zarr or InMemoryGeff conforms to geff specification

    Args:
        input (str | Path | zarr store | InMemoryGeff): Check the geff zarr or memory geff

    Raises:
        ValueError: If geff specs are violated
        FileNotFoundError: If store is not a valid zarr store or path doesn't exist
    """
    if type(input) is InMemoryGeff:
        ...
        # validate_structure_mem_geff(input)
    else:
        StoreValidator.validate_structure(input)


########## validate on storelike #################


class StoreValidator:
    @classmethod
    def validate_structure(cls, store: StoreLike) -> None:
        ...
        cls._validate_nodes_group(nodes_group, metadata)

        edges_group = expect_group(graph_group, _path.EDGES)
        cls._validate_edges_group(edges_group, metadata)

        # Metadata based validation
        if metadata.axes is not None:
            cls._validate_axes_structure(graph_group, metadata)
        ...
        for prop_name in props_group.keys():
            ...
            if prop_metadata.varlength:
                ...
                if _path.DATA in arrays:
                    raise ValueError(
                        f"Found data array for property {prop_name} "
                        "which is not a varlength property"
                    )
    ...
    @classmethod
    def _validate_nodes_group(cls, nodes_group: zarr.Group, metadata: GeffMetadata) -> None:
        ...
        cls._validate_props_group(node_props, id_len, "Node", metadata.node_props_metadata)

    @classmethod
    def _validate_edges_group(cls, edges_group: zarr.Group, metadata: GeffMetadata) -> None:
        ...
        if edges_ids.ndim != 2 or edges_ids.shape[-1] != 2:
            raise ValueError(
                f"edges ids must be 2d with last dimension of size 2, "
                f"received shape {edges_ids.shape}"
            )
        ...
        if not isinstance(edge_props, zarr.Group):
            raise ValueError(
                f"{_path.EDGES!r} group must contain a {_path.PROPS!r} group. "
                f"Got {type(edge_props)}"
            )
        cls._validate_props_group(edge_props, edge_id_len, "Edge", metadata.edge_props_metadata)
