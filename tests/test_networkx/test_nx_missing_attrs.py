...
def test_missing_pos_prop(tmp_path):
    ...
    graph, _ = graph_sparse_node_props()
    # wrong property name
    with pytest.raises(ValueError, match=r"Spatiotemporal property .* not found in node .*"):
        geff.write_nx(graph, axis_names=["t", "y", "z"], path=zarr_path)
    # missing property
    del graph.nodes[1]["t"]
    print(graph.nodes[1])
    zarr_path = Path(tmp_path) / "test2.zarr"
    with pytest.raises(ValueError, match=r"Spatiotemporal property t not found in node 1"):
        geff.write_nx(graph, axis_names=["t", "y", "x"], path=zarr_path)
